...
def search_cards(request, query: str):
    container = containers.get_container()
    use_case = SearchCardsUseCase(card_service=container.resolve(BaseCardService))
    try:
        cards = use_case.execute(query=query)
        items = [GuideSchema.from_entity(card) for card in cards]
    except ServiceException as e:
        return HttpError(status_code=400, message=e.message)
...
@router.get(
    "/{id}", response=ApiResponse[GuideSchema], summary="Получить информацию о карточке"
)
